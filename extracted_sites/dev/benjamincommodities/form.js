...
(function () {
        ...
        for (let i = 0; i < elements.length; i++) {
            ...
            if (targetField) {
                ...
                if (element.type === 'checkbox') {
                    ...
                    log('Checkbox field:', fieldIdentifier, '→', targetField, '= checked:', value);
    ...
    function handleFormSubmit(form) {
        form.addEventListener('submit', async function (e) {
            e.preventDefault();
            e.stopPropagation();
            e.stopImmediatePropagation(); // Stop Gravity Forms and Elementor handlers

            log('Form submitted - event captured');
            ...
            // Find submit button (support Gravity Forms, Elementor, and other form builders)
            log('Looking for submit button in form...');
            const submitBtn = form.querySelector('button[type="submit"]') ||
                form.querySelector('input[type="submit"]') ||
                form.querySelector('[id*="gform_submit_button"]') ||
                form.querySelector('.elementor-button[type="submit"]') ||
                form.querySelector('button.elementor-button');

            if (!submitBtn) {
                log('WARNING: Submit button not found! Trying all buttons...');
                const allButtons = form.querySelectorAll('button');
                log('Found', allButtons.length, 'button elements');
                allButtons.forEach((btn, idx) => {
                    log(`Button ${idx}:`, 'type=', btn.type, 'class=', btn.className, 'text=', btn.textContent?.trim().substring(0, 30));
                });
            } else {
                log('Submit button found:', submitBtn.tagName, 'type=', submitBtn.type, 'class=', submitBtn.className);
            }
    ...
    function initializeForms() {
        ...
        forms.forEach((form, index) => {
            ...
            // For Elementor Forms, intercept button click before Elementor handlers
            if (form.classList.contains('elementor-form') && window.DISABLE_ELEMENTOR_FORMS) {
                log('Detected Elementor form - intercepting button click');

                const submitButtons = form.querySelectorAll('button[type="submit"], .elementor-button');
                submitButtons.forEach(function(btn) {
                    ...
                    newBtn.addEventListener('click', function(e) {
                        ...
                        log('Button clicked - triggering manual form submit');

                        // Manually trigger our submit handler
                        form.dispatchEvent(new Event('submit', { bubbles: true, cancelable: true }));
                    });
                });
            }

            // For Forminator Forms, intercept button click before Forminator handlers
            if (form.classList.contains('forminator-custom-form') && window.DISABLE_FORMINATOR_FORMS) {
                log('Detected Forminator form - intercepting button click');

                const submitButtons = form.querySelectorAll('button[type="submit"], .forminator-button-submit');
                submitButtons.forEach(function(btn) {
                    // Remove all existing click handlers by cloning
                    const newBtn = btn.cloneNode(true);
                    btn.parentNode.replaceChild(newBtn, btn);
                    ...
                    newBtn.addEventListener('click', function(e) {
                        ...
                        e.stopPropagation();
                        e.stopImmediatePropagation();
                        log('Forminator button clicked - triggering manual form submit');
    ...
    log('Generic Form Handler loaded (supports Gravity Forms, Contact Form 7, Elementor, and custom HTML forms)');
