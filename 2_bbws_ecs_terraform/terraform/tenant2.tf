...
resource "random_password" "tenant_2_db" {
  length  = 24
  special = false
}

# Secrets Manager secret for tenant-2 database credentials
resource "aws_secretsmanager_secret" "tenant_2_db" {
  name        = "${var.environment}-tenant-2-db-credentials"
...
resource "aws_secretsmanager_secret_version" "tenant_2_db" {
  secret_id = aws_secretsmanager_secret.tenant_2_db.id
  secret_string = jsonencode({
    username = "tenant_2_user"
    password = random_password.tenant_2_db.result
...
resource "aws_efs_access_point" "tenant_2" {
  ...
  tags = {
    Name        = "${var.environment}-tenant-2-ap"
    Environment = var.environment
    Tenant      = "tenant-2"
  }
}
...
resource "aws_ecs_task_definition" "tenant_2" {
  family                   = "${var.environment}-tenant-2"
  network_mode             = "awsvpc"
    ...
    secrets = [
      {
        ...
        valueFrom = "${aws_secretsmanager_secret.tenant_2_db.arn}:username::"
      },
      {
        name      = "WORDPRESS_DB_PASSWORD"
        valueFrom = "${aws_secretsmanager_secret.tenant_2_db.arn}:password::"
      }
    ]
  ...
  volume {
    ...
    efs_volume_configuration {
      ...
      authorization_config {
        access_point_id = aws_efs_access_point.tenant_2.id
        iam             = "ENABLED"
      }
    }
  }
  ...
  tags = {
    Name        = "${var.environment}-tenant-2-task"
    Environment = var.environment
    Tenant      = "tenant-2"
  }
}
...
resource "aws_lb_target_group" "tenant_2" {
  name        = "${var.environment}-tenant-2-tg"
  port        = 80
  protocol    = "HTTP"
...
resource "aws_lb_listener_rule" "tenant_2" {
  listener_arn = aws_lb_listener.http.arn
  priority     = 20

  action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.tenant_2.arn
  }

  condition {
    host_header {
      values = ["tenant2.wpdev.kimmyai.io"]
    }
  }

  tags = {
    Name        = "${var.environment}-tenant-2-rule"
    Environment = var.environment
    Tenant      = "tenant-2"
  }
}
...
resource "aws_ecs_service" "tenant_2" {
  name            = "${var.environment}-tenant-2-service"
  cluster         = aws_ecs_cluster.main.id
  task_definition = aws_ecs_task_definition.tenant_2.arn
  desired_count   = 1
  launch_type     = "FARGATE"
  ...
  depends_on = [
    aws_lb_listener_rule.tenant_2,
    null_resource.tenant_2_db_init
  ]
...
resource "null_resource" "tenant_2_db_init" {
  depends_on = [
    aws_db_instance.main,
    aws_secretsmanager_secret_version.tenant_2_db,
  ...
  triggers = {
    db_secret = aws_secretsmanager_secret_version.tenant_2_db.id
  }
}

# Update IAM policy to allow access to tenant-2 secret
resource "aws_iam_role_policy" "ecs_task_execution_secrets_tenant_2" {
  name = "${var.environment}-ecs-secrets-access-tenant-2"
  ...
  policy = jsonencode({
    ...
    Statement = [{
      ...
      Resource = [
        aws_secretsmanager_secret.tenant_2_db.arn,
        "${aws_secretsmanager_secret.tenant_2_db.arn}*"
      ]
    }]
  })
...
resource "aws_iam_role_policy" "ecs_task_efs_tenant_2" {
  name = "${var.environment}-ecs-efs-access-tenant-2"
  ...
  policy = jsonencode({
    ...
    Statement = [{
      ...
      Condition = {
        StringEquals = {
          "elasticfilesystem:AccessPointArn" = aws_efs_access_point.tenant_2.arn
        }
      }
    }]
  })
