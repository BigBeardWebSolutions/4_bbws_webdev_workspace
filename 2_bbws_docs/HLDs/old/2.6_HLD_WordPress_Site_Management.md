...
- Site lifecycle management (create, configure, clone, delete)
...
|--------|------------------|---------|
| **Tenant Management (BRS 2.5)** | Internal API | Validate tenant context, user permissions |
| **Instance Management (BRS 2.7)** | Internal API | Access WordPress container endpoints |
...
### 4.1 Site Creation Flow

```
┌─────────┐    ┌─────────────┐    ┌────────────────┐    ┌──────────────┐
│  User   │    │  Sites API  │    │ WordPress Proxy│    │   WordPress  │
└────┬────┘    └──────┬──────┘    └───────┬────────┘    └──────┬───────┘
     │                │                    │                    │
     │  POST /sites   │                    │                    │
     │───────────────>│                    │                    │
...
```
┌─────────┐   ┌─────────────┐   ┌────────┐   ┌────────────────┐   ┌───────────┐
│  User   │   │Templates API│   │  Queue │   │ Async Processor│   │ WordPress │
└────┬────┘   └──────┬──────┘   └───┬────┘   └───────┬────────┘   └─────┬─────┘
     │               │              │                │                  │
     │ POST /apply   │              │                │                  │
     │──────────────>│              │                │                  │
...
```
┌─────────┐   ┌───────────┐   ┌────────┐   ┌────────────────┐   ┌─────────────┐
│  User   │   │ Sites API │   │  Queue │   │ Async Processor│   │ Destination │
└────┬────┘   └─────┬─────┘   └───┬────┘   └───────┬────────┘   │ Environment │
     │              │             │                │            └──────┬──────┘
     │ POST /promote│             │                │                   │
     │─────────────>│             │                │                   │
...
### 5.1 WordPress REST API Integration
...
|---------|-------------|---------|
| **Tenant API (BRS 2.5)** | REST | Validate tenant exists and user has permissions |
| **Instance API (BRS 2.7)** | REST | Get WordPress container endpoint for tenant |
...
```
┌─────────┐    ┌─────────────┐    ┌────────────┐    ┌─────────────┐
│  User   │    │  API Gateway│    │   Cognito  │    │  Sites API  │
└────┬────┘    └──────┬──────┘    └─────┬──────┘    └──────┬──────┘
     │                │                 │                  │
     │  Request with  │                 │                  │
     │  JWT token     │                 │                  │
...
| OpenAPI Spec | `LLDs/openapi/wp_technical_api_openapi.yaml` |
