"""
OrderInternalNotificationSender Lambda Handler.

Processes SQS messages for new orders and sends internal notification emails.
...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    """
    Lambda handler for OrderInternalNotificationSender.

    Processes SQS messages containing order creation events and sends
    internal notification emails to the team.
    ...
    """
    logger.info(f"OrderInternalNotificationSender invoked: {len(event.get('Records', []))} records")
    ...
    for record in event.get('Records', []):
        ...
        try:
            ...
            message_id_ses = email_service.send_internal_notification(order)
            logger.info(f"Internal notification sent for order {order_id}: SES MessageId={message_id_ses}")
